...
function ReaderBaseFrame(callback) {
    function parseChapterData(jsonData) {
   
			var jsonObj=$api.getStorage('shuju');
		 alert($api.jsonToStr(jsonObj));
...
//     alert($api.jsonToStr(jsonObj));
        ...
        var cur=0;
        var add=0;
        var fontsize = Util.StorageGetter('font_size');
        alert(jsonObj.p.length);
        for (var i = 0; i < jsonObj.p.length; i++) {
            
            addhtml += '<p>' + jsonObj.p[i] + '</p>';
            
            cur+=jsonObj.p[i].length;
            
            if(cur>=285){
               cur=0;
               if(add == 0){
	               html+='<div class="m-page m-bigTxt f-hide page1" data-page-type="bigTxt" data-statics="info_list" style="background:#fff;height: 907px;">'+
				                '<div class="page-con j-txtWrap lazy-finish" data-position="50% 50%" data-size="cover" style="background-size: cover; background-position: 50% 50%;">'+
					                '<div id="fition_chapter_title"></div>'+
								    '<div id="fiction_container" class="m-read-content" style="-webkit-user-select: all;">'+
								        '<h4>' + jsonObj.t + '</h4>'+
								        addhtml+
								    '</div>'+
				                '</div>'+
			       ...
			       html+='<div class="m-page m-bigTxt f-hide page1" data-page-type="bigTxt" data-statics="info_list" style="height: 907px;">'+
				                '<div class="page-con j-txtWrap lazy-finish" data-position="50% 50%" data-size="cover" style="background-size: cover; background-position: 50% 50%;">'+
					                '<div id="fition_chapter_title"></div>'+
								    '<div id="fiction_container" class="m-read-content" style="-webkit-user-select: all;">'+
								        addhtml+
...
//      html+='<div class="m-page m-bigTxt f-hide" data-page-type="bigTxt" data-statics="info_list" style="height: 907px;">'+
//              '<div class="page-con j-txtWrap lazy-finish" data-position="50% 50%" data-size="cover" style="background-size: cover; background-position: 50% 50%;">'+
//	                '<div id="fition_chapter_title"></div>'+
//				    '<div id="fiction_container" class="m-read-content" style="-webkit-user-select: all;">'+
//				        '<h4>第五章:天外飞仙</h4>'+
//				        '<p>龙雪傲得到了远古巨龙的传承，传承血炼诀与上古星空炼神诀。在异龙血脉觉醒后，龙雪傲踏上了征服世界的巅峰之路，最后得以战魂不败的永生。他仰天长啸：我命由我不由天！神挡诛神，魔挡灭魔'+
//							 '成神？抑或成魔？'+
//							 '我神即我身，游地狱血海，历千险万劫，纵使魂飞魄散，我神识依在，战百世轮回，纵使六道无常，我依然永生'+
//							 '我命由我不由天！神挡诛神，魔挡灭魔！'+
//							 '战魂觉醒，万面齐聚，轮回启，神魂消，人魂散，战魂依然不败！ '+
//							'初炼之旅 第一章：天赋测试.............'+
//							 '</p>'+
//					    '<div class="need_pay">'+
//					        '<div class="need_pay_title">'+
//					           '<p>价格:<span>8书币</span></p>'+
//					           '<p>金币:<span>0书币</span></p>'+
//					        '</div>'+
//					        '<p class="need_pay_content">购买此章节,感谢作者,支持正版阅读</p>'+
//					        '<div class="need_pay_footer buy_que"> '+
//					            '您的金币不足,请充值后购买'+
//					        '</div>'+
//					    '</div>'+
//				    '</div>'+
...
//          '</div>';
    ...
    return function (data) {
        callback&&callback(parseChapterData(data));
    }
}
 ...
 *************************/
    ...
    addPage: function(data,callback){
       var html = "";
       var $rootNode = $('.translate-back');//书本内容展示
       var ReaderUI = ReaderBaseFrame(function(html){
           
           var roothtml = $rootNode.html();
                
           roothtml = roothtml+html;
           
           $rootNode.html(roothtml);
           var initBackground = Util.StorageGetter('background');
           $('.m-page').css('background','url('+initBackground+')');
           car2._pageNum=$('.m-page').size();
	       car2._page = $('.m-page');
	       car2._rotateNode = $('.p-ct');
	       car2.page_stop();
	       car2.page_start();
	       callback&&callback();
       });
    ...
    changeFontSize:function(jsonData,fontsize,callback){
       var $curPages = $('.page1');
       var allpage = $curPages.length;
       var jsonObj = JSON.parse(jsonData);
       var needCount;
       var nowCount=0;
       var nowIndex=0;
       ...
       for(var i = 0; i < jsonObj.p.length; i++) {
          nowCount += jsonObj.p[i].length;
          Curcontent+='<p>' + jsonObj.p[i] + '</p>';
          if(nowCount>=needCount){
             pageArr[nowIndex] = Curcontent;
             nowIndex++;
             nowCount=0;
             Curcontent="";
          }
       ...
       if(nowIndex > allpage){
           for(var i=0;i<nowIndex-allpage;i++){
               var $curPages = $('.page1');
               var $lastDom = $curPages.eq($curPages.length-1);
               var $inserDom = $('<div class="m-page m-bigTxt f-hide page1" data-page-type="bigTxt" data-statics="info_list" style="height: 907px;">'+
				                '<div class="page-con j-txtWrap lazy-finish" data-position="50% 50%" data-size="cover" style="background-size: cover; background-position: 50% 50%;">'+
					                '<div id="fition_chapter_title">1243</div>'+
								    '<div id="fiction_container" class="m-read-content" style="-webkit-user-select: all;">'+
           ...
           for(var i=0;i<allpage-nowIndex;i++){
               var $curPages = $('.page1');
               $curPages.eq($curPages.length-1).remove();
           }
       }
       
       var $curPages = $('.page1');
       var initBackground = Util.StorageGetter('background');
       var initFontSise = Util.StorageGetter('font_size');
	   $curPages.find('.m-read-content').css('font-size', initFontSise+'px');
       $('.m-page').css('background','url('+initBackground+')');
       car2._pageNum=$('.m-page').size();
       car2._page = $('.m-page');
       car2._rotateNode = $('.p-ct');
       ...
       for(var i=0;i<$curPages.length;i++){
           if(i == 0){
              ...
              html+=pageArr[i];
              var $ct = $curPages.eq(i).find('.m-read-content');
              $ct.html(html);
           }else{
	           var $ct = $curPages.eq(i).find('.m-read-content');
 ...
 */
 	...
 	page_touch_start: function(e){
 		...
 		if(e.type == "touchstart"){
        	car2._touchStartValX = window.event.touches[0].pageX;
        }else{
        	car2._touchStartValX = e.pageX||e.x;
 	...
 	page_touch_move : function(e){
        ...
        //$self.css('backgroundImage','url('+initBackground+')');
 	...
 	page_position	: function(e,moveP,$self){ 		
 		...
 		// 设置移动的距离
 		if(moveP!='undefined') car2._touchDeltaX = moveP - car2._touchStartValX;

 		// 设置移动方向
    	car2._movePosition = moveP - car2._touchStartValX >0 ? 'left' : 'right';
        ...
        if(car2._touchDeltaX<=0){
 		...
 		function init_next(node){
			...
			$(node[0]).css('zIndex',9);
			$(node[1]).css('zIndex',40);
			
	 		// 设置下一页面的显示和位置        
	        if(car2._movePosition=='right'){
 				s = parseInt($(window).scrollTop());
 				if(s>0) l = $(window).width();
 				else l = $(window).width();
 				node[0].style[car2._prefixStyle('transform')] = 'translate(0,0)'+_translateZ;
 				$(node[0]).attr('data-translate',l);

 				$(node[1]).attr('data-translate',0);
	 		}else{
 				node[0].style[car2._prefixStyle('transform')] = 'translate(0px,0px)'+_translateZ;
				l = $(window).width()
 				$(node[0]).attr('data-translate',l);
 				$(node[1]).attr('data-translate',0);
	 		}
 	...
 	page_translate	: function(node){
		...
		// 当前的页面移动
		if(car2._movePosition == 'left'){
			node[1].style[car2._prefixStyle('transform')] = 'translate(0,0)'+_translateZ;
			$(node[0]).css('zIndex',42);
			if($(node[0]).attr('data-translate')) y_2 =-parseInt($(node[0]).attr('data-translate'))+y;
			node[0].style[car2._prefixStyle('transform')] = 'translate('+y_2+'px,0)'+_translateZ;
			//node[0].style.background = "none";
		}else{
			if($(node[1]).attr('data-translate')) y_2 = y + parseInt($(node[1]).attr('data-translate'));
			node[1].style[car2._prefixStyle('transform')] = 'translate('+y_2+'px,0)'+_translateZ;
		}
 	...
 	page_touch_end	: function(e){
		...
		// 页面切换
 		if(Math.abs(car2._touchDeltaX)>=5){		// 切换成功
 			car2.page_success();
 		}else if(Math.abs(car2._touchDeltaX)>10&&Math.abs(car2._touchDeltaX)<50){	// 切换失败		
 			car2.page_fial();
 		}else{									// 没有切换
 			car2.page_fial();
 		}
 	...
 	page_success	: function(){
 		...
 		// 下一个页面的移动
		if(car2._movePosition == 'left'){
			car2._page.eq(car2._pageNext)[0].style['transition']='all .5s';
 		    car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transform')] = 'translate(0,0)'+_translateZ;
		}else{
			car2._page.eq(car2._pageNow)[0].style['transition']='all .5s';
			var l = $(window).width();
			l = -l;
			car2._page.eq(car2._pageNow)[0].style[car2._prefixStyle('transform')] = 'translate('+l+'px,0)'+_translateZ;
		}
 		// 成功事件
 	...
 	page_fial	: function(){
		...
		if(!car2._pageNext&&car2._pageNext!=0) {
         ...
         car2._page.eq(car2._pageNext)[0].style['transition']='all .2s';
 		if(car2._movePosition=='left'){
 		}else{
 		}
