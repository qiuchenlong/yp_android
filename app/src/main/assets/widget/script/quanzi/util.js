(function(window){
	...
	util.openImageBrower = function(images,index){
	...
	util.downLoadImage = function(url,callback){
        
	    api.download({
		    url: url,
		    savePath: 'fs://touxiang/'+url+'.png',
		    report: true,
		    allowResume: true,
		    cache:true
		}, function(ret, err) {
		    ...
		    if (ret.state == 1) {
		       callback&&callback(ret['savePath']);
		     
		    } 
		});
	}
	
	util.encrypt = function(msg){
	    var rsa = new JSEncrypt();
	    rsa.setPublicKey('-----BEGIN PUBLIC KEY-----MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCpvdExvx5zybwFboU1NSjuIghiuXNQSNiJ27G8iETbJdhtibKvTlbyzBVhq0brhnG16MG29pf8055zwODYKpWC+hPVJTflzhcYEVEqRpstq9q2BHe75ErRgjYIQiNGZJ11czQUj82TgDP3xMKntpchel5J3XbYfGjMzG3nARoOpwIDAQAB-----END PUBLIC KEY-----', '10001'); 
	 	return rsa.encrypt(msg); 
	}
	
	util.mgetDay = function(day){  
	       var today = new Date();  
	       
	       var targetday_milliseconds=today.getTime() + 1000*60*60*24*3*day;          
...
//		 });
	...
	util.longAjax1 = function(url,time,callback){
	     var one = 0;
	     var two = 0;
	     var three = 0;
